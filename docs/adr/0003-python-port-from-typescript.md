...
1. **Bootstrap** this repository (`bq-inspector` on PyPI, import name `bq_inspector`) as a Python port of the TypeScript CLI, targeting **behavioral parity** with the existing command surface, `--params` JSON contract, schema flags, and error codes.
2. **CLI entry point**: `[project.scripts]` maps `bq-inspector` to `bq_inspector.cli:main`, matching the npm binary name `bq-inspector`.
...
- Python agents gain a native install path (`pip install bq-inspector`) without Node.

### Contract notes (Python vs TypeScript)

- **HTTP 4xx** from BigQuery (except 401, 403, 404, 429) map to `BQINSPECTOR_INPUT_INVALID` (non-retriable, exit 2). The TypeScript CLI maps other unlisted 4xx to `BQINSPECTOR_API_UNAVAILABLE` (retriable).
...
- **`BQINSPECTOR_JOB_NOT_FOUND`** is emitted for every HTTP 404 (jobs and catalog APIs), matching TypeScript. Catalog 404 responses include an extra stderr `hint`; agents should read `source.api`.
...
- **jsonschema vs Ajv**: Slightly different validation messages and draft feature support; tests must assert stable error paths (`BQINSPECTOR_INPUT_INVALID`) rather than byte-identical Ajv output.
