...
_STATUS_CODE_RULES: tuple[tuple[Callable[[int], bool], BqInspectErrorCode], ...] = (
    (lambda status: status in (401, 403), "BQINSPECT_PERMISSION_DENIED"),
    (lambda status: status == _HTTP_STATUS_NOT_FOUND, "BQINSPECT_JOB_NOT_FOUND"),
    (lambda status: status == _HTTP_STATUS_TOO_MANY_REQUESTS, "BQINSPECT_API_RATE_LIMITED"),
    (lambda status: status >= _HTTP_STATUS_SERVER_ERROR, "BQINSPECT_API_UNAVAILABLE"),
    (
