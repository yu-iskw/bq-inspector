...
Examples:
  bq-inspect {command} --params '{meta.example_inline}'
  bq-inspect {command} --params @{meta.example_file}
""".strip()
...
def build_schema_subcommand_usage(name: Literal["input", "output"]) -> str:
    ...
    return f"""
Usage:
  bq-inspect schema {name} --format json-schema
""".strip()
