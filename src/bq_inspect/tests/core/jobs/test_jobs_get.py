...
async def test_records_validation_errors_for_invalid_job_references() -> None:
    ...
    assert response["jobs"][0]["errors"][0]["code"] == "BQINSPECT_INPUT_INVALID"
    assert len(response["errors"]) == 1
...
@pytest.mark.asyncio
async def test_records_bq_inspect_failure_from_get_job() -> None:
    class DenyClient:
    ...
    assert response["jobs"][0]["errors"][0]["code"] == "BQINSPECT_JOB_NOT_FOUND"
...
async def test_maps_unexpected_get_job_errors_to_internal_errors() -> None:
    ...
    assert response["jobs"][0]["errors"][0]["code"] == "BQINSPECT_INTERNAL"
