...
if TYPE_CHECKING:
    from bq_inspector.core.shared.types import BqInspectErrorCode, JobRef
...
_CATALOG_NOT_FOUND_HINT = (
    ...
    "BQINSPECTOR_JOB_NOT_FOUND is used for all HTTP 404 responses; inspect source.api "
...
def hint_for_api_error(
    ...
    parts: list[str] = []
    iam_hint = iam_hint_for_api(api) if code == "BQINSPECTOR_PERMISSION_DENIED" else None
    ...
    if (
        ...
        and code in ("BQINSPECTOR_PERMISSION_DENIED", "BQINSPECTOR_JOB_NOT_FOUND")
    ):
        parts.append(_MISSING_JOB_LOCATION_HINT)

    if (
        api == "bigquery.jobs.get"
        and code == "BQINSPECTOR_PERMISSION_DENIED"
        and not _is_job_location_missing(job_ref)
    ):
        parts.append(_JOBS_GET_PERMISSION_DENIED_WITH_LOCATION_HINT)

    if code == "BQINSPECTOR_JOB_NOT_FOUND" and api in _CATALOG_APIS:
