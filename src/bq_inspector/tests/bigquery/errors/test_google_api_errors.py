...
from bq_inspector.bigquery.errors.google_api_errors import (
    extract_google_error_message,
    map_google_error_to_bq_inspector_failure,
...
@pytest.mark.parametrize(
    ("status", "code"),
    [
        (401, "BQINSPECTOR_PERMISSION_DENIED"),
        (403, "BQINSPECTOR_PERMISSION_DENIED"),
        (404, "BQINSPECTOR_JOB_NOT_FOUND"),
        (429, "BQINSPECTOR_API_RATE_LIMITED"),
        (500, "BQINSPECTOR_API_UNAVAILABLE"),
        (400, "BQINSPECTOR_INPUT_INVALID"),
        (409, "BQINSPECTOR_INPUT_INVALID"),
        (422, "BQINSPECTOR_INPUT_INVALID"),
        (418, "BQINSPECTOR_INPUT_INVALID"),
    ],
)
...
def test_map_google_error_to_bq_inspector_failure_maps_http_errors() -> None:
    failure = map_google_error_to_bq_inspector_failure(
        {"code": 404, "message": "Missing job."},
        "bigquery.jobs.get",
    )

    assert isinstance(failure, BqInspectFailure)
    assert failure.details["code"] == "BQINSPECTOR_JOB_NOT_FOUND"
    assert failure.details["source"] == {"api": "bigquery.jobs.get", "status": 404}


def test_map_google_error_to_bq_inspector_failure_adds_iam_hints_for_catalog_apis() -> None:
    failure = map_google_error_to_bq_inspector_failure(
        {"code": 403, "message": "Denied."},
        "bigquery.tables.get",
    )

    assert failure.details["code"] == "BQINSPECTOR_PERMISSION_DENIED"
...
def test_map_google_error_to_bq_inspector_failure_adds_location_guidance_for_jobs_get_403() -> None:
    failure = map_google_error_to_bq_inspector_failure(
...
def test_map_google_error_to_bq_inspector_failure_adds_location_guidance_for_jobs_get_404() -> None:
    failure = map_google_error_to_bq_inspector_failure(
...
def test_map_google_error_to_bq_inspector_failure_maps_client_errors_as_non_retriable() -> None:
    failure = map_google_error_to_bq_inspector_failure(
        {"code": 400, "message": "Invalid state filter."},
        "bigquery.jobs.list",
    )

    assert failure.details["code"] == "BQINSPECTOR_INPUT_INVALID"
    assert failure.details["retriable"] is False
    assert failure.details["source"] == {"api": "bigquery.jobs.list", "status": 400}


def test_map_google_error_to_bq_inspector_failure_maps_missing_status_to_internal() -> None:
    failure = map_google_error_to_bq_inspector_failure(RuntimeError("network"))

    assert failure.details["code"] == "BQINSPECTOR_INTERNAL"
